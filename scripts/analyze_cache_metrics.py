...
def print_report(metrics: CacheMetrics, period_days: int = 1) -> None:
    ...
    print(f"  Cost per Call:      $0.02 (estimated)")
    ...
    print(f"  vs LLM Response:    ~2,000-5,000ms")
    ...
    print(f"  Redis Stack:        {'✓ Available' if metrics.redis_stack_available else '✗ Fallback mode'}")
...
Examples:
  ...
  python scripts/analyze_cache_metrics.py --json > metrics.json
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Analysis period in days (default: 1)"
    )
    parser.add_argument(
        "--json", "-j",
        action="store_true",
    ...
    parser.add_argument(
        "--quiet", "-q",
        action="store_true",
        help="Only output numbers (for scripting)"
    )
