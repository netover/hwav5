...
from resync.knowledge.ingestion.extractor import ALLOWED_RELATIONS, Triplet, TripletExtractor
from resync.knowledge.retrieval.graph import TWSKnowledgeGraph
from resync.knowledge.retrieval.hybrid import HybridRAG, QueryClassifier, QueryIntent
from resync.knowledge.models import NodeType, RelationType
...
class TestKnowledgeGraphBasic:
    ...
    async def test_add_node_to_graph(self, knowledge_graph):
        ...
        with patch("resync.knowledge.retrieval.graph.get_async_session") as mock_sess:
            ...
            node_id = await knowledge_graph.add_node(
                "job:TEST_JOB", NodeType.JOB, "TEST_JOB", properties={"description": "Test job"}
            )
    ...
    async def test_add_edge_to_graph(self, knowledge_graph):
        ...
        with patch("resync.knowledge.retrieval.graph.get_async_session") as mock_sess:
            ...
            # Add edge
            await knowledge_graph.add_edge("job:A", "job:B", RelationType.DEPENDS_ON)
...
class TestHybridRAG:
    ...
    async def test_query_routes_to_graph(self, hybrid_rag):
        ...
        with patch.object(hybrid_rag, "_get_kg") as mock_kg:
            ...
            mock_graph.get_dependency_chain = AsyncMock(
                return_value=[{"from": "job:A", "to": "job:B", "relation": "depends_on"}]
            )
            mock_kg.return_value = mock_graph

            result = await hybrid_rag.query(
                "Quais as dependências de JOB_A?", generate_response=False
            )
    ...
    async def test_query_routes_to_rag(self, hybrid_rag):
        ...
        with patch.object(hybrid_rag, "_get_rag") as mock_rag_fn:
            ...
            mock_rag.retrieve = AsyncMock(
                return_value=[{"text": "Documentation content", "source": "manual.md"}]
            )
            mock_rag_fn.return_value = mock_rag

            result = await hybrid_rag.query(
                "Como configurar o agente TWS?", generate_response=False
            )
    ...
    async def test_full_workflow(self, knowledge_graph):
        ...
        with patch("resync.knowledge.retrieval.graph.get_async_session") as mock_sess:
            ...
            await knowledge_graph.add_job(
                "EXTRACT_DATA", workstation="WS001", resources=["DB_SOURCE"]
            )
            await knowledge_graph.add_job(
                "TRANSFORM_DATA",
                workstation="WS002",
            ...
            conflicts = await knowledge_graph.find_resource_conflicts("TRANSFORM_DATA", "LOAD_DATA")
