...
module_path = os.path.join(
    os.path.dirname(__file__), "..", "..", "resync", "core", "circuit_breakers.py"
)
...
def test_circuit_breaker_configuration():
    ...
    assert "AuthenticationError" in [str(e) for e in tws_breaker.exclude] or any(
        isinstance(e, type) and e.__name__ == "AuthenticationError" for e in tws_breaker.exclude
    )
