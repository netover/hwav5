...
async def test_redis_breaker_listener_logs_state_changes(mock_logger, mock_runtime_metrics):
    ...
    mock_runtime_metrics.record_health_check.assert_called_once_with(
        "redis_circuit_breaker", "opened", {"failure_count": 3}
    )


@pytest.mark.asyncio
async def test_initialize_redis_with_retry_circuit_breaker_success(
    mock_redis_connection_manager, mock_redis_client
):
...
@pytest.mark.asyncio
async def test_initialize_redis_with_retry_circuit_breaker_failure(
    mock_redis_connection_manager, mock_redis_client
):
...
@pytest.mark.asyncio
async def test_initialize_redis_with_retry_auth_error_not_protected(
    mock_redis_connection_manager, mock_redis_client
):
...
async def test_circuit_breaker_listener_integration(mock_logger, mock_runtime_metrics):
    ...
    # Verify metrics were recorded
    mock_runtime_metrics.record_health_check.assert_called_once_with(
        "redis_circuit_breaker", "opened", {"failure_count": 5}
    )
...
async def test_circuit_breaker_decorator_applied_to_initialize_redis_with_retry():
    ...
    assert hasattr(initialize_redis_with_retry, "__wrapped__") or hasattr(
        initialize_redis_with_retry, "call"
    )
...
@pytest.mark.asyncio
async def test_circuit_breaker_does_not_interfere_with_connection_pooling(
    mock_redis_connection_manager, mock_redis_client
):
