...
class TestAgentResponseStreamerHappyPath:
    ...
    @pytest.mark.asyncio
    async def test_stream_response_with_streaming_agent(self, mock_websocket, mock_agent_streaming):
    ...
    @pytest.mark.asyncio
    async def test_stream_response_sends_chunks(self, mock_websocket, mock_agent_streaming):
    ...
    @pytest.mark.asyncio
    async def test_stream_response_sends_stream_end(self, mock_websocket, mock_agent_streaming):
        ...
        stream_end_calls = [call for call in calls if call[0][0].get("type") == "stream_end"]
...
class TestAgentResponseStreamerNonStreaming:
    ...
    async def test_stream_response_non_streaming_agent(
        ...
        # Act
        result = await streamer.stream_response(mock_agent_non_streaming, "Test query")
...
class TestAgentResponseStreamerErrors:
    ...
    async def test_websocket_disconnect_during_streaming(
        ...
        # Arrange
        mock_websocket.send_json = AsyncMock(side_effect=WebSocketDisconnect(code=1001))
        streamer = AgentResponseStreamer(mock_websocket)
    ...
    async def test_error_sending_error_message(self, mock_websocket):
        ...
        mock_websocket.send_json = AsyncMock(side_effect=Exception("WebSocket closed"))
...
class TestAgentResponseStreamerMessages:
    ...
    @pytest.mark.asyncio
    async def test_stream_message_format(self, mock_websocket, mock_agent_streaming):
    ...
    @pytest.mark.asyncio
    async def test_stream_end_message_format(self, mock_websocket, mock_agent_streaming):
