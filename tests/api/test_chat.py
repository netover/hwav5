...
class TestGetEnhancedQuery:
    ...
    async def test_get_enhanced_query_success(self, mock_knowledge_graph):
        ...
        result = await _get_enhanced_query(
            mock_knowledge_graph, "query", "original"
        )
    ...
    async def test_get_enhanced_query_formats_correctly(self, mock_knowledge_graph):
        ...
        result = await _get_enhanced_query(
            mock_knowledge_graph, "sanitized", "original"
        )
...
class TestFinalizeAndStoreInteraction:
    ...
    async def test_finalize_sends_final_message(
        ...
        await _finalize_and_store_interaction(
            mock_websocket, mock_knowledge_graph, mock_agent,
            "agent_id", "query", "response"
        )
    ...
    async def test_finalize_handles_agent_without_attributes(
        ...
        await _finalize_and_store_interaction(
            mock_websocket, mock_knowledge_graph, minimal_agent,
            "agent_id", "query", "response"
        )
...
class TestHandleAgentInteraction:
            ...
            with patch('resync.api.chat._get_optimized_response') as mock_opt:
                with patch('resync.api.chat._finalize_and_store_interaction') as mock_fin:
                    ...
                    await _handle_agent_interaction(
                        mock_websocket, mock_agent, "agent_id",
                        mock_knowledge_graph, "Query about job status"
                    )
            ...
            with patch('resync.api.chat._get_enhanced_query') as mock_enh:
                with patch('resync.api.chat.AgentResponseStreamer') as mock_stream:
                    ...
                    with patch('resync.api.chat._finalize_and_store_interaction'):
                        await _handle_agent_interaction(
                            mock_websocket, mock_agent, "agent_id",
                            mock_knowledge_graph, "General query"
                        )
    ...
    async def test_handle_interaction_sends_user_message(
        ...
        with patch('resync.api.chat._should_use_llm_optimization', return_value=True):
            with patch('resync.api.chat._get_optimized_response', return_value=AsyncMock(return_value="Response")):
                with patch('resync.api.chat._finalize_and_store_interaction'):
                    await _handle_agent_interaction(
                        mock_websocket, mock_agent, "agent_id",
                        mock_knowledge_graph, "User message"
                    )
            ...
            with patch('resync.api.chat._should_use_llm_optimization', return_value=True):
                with patch('resync.api.chat._get_optimized_response', return_value=AsyncMock(return_value="Response")):
                    with patch('resync.api.chat._finalize_and_store_interaction'):
                        await _handle_agent_interaction(
                            mock_websocket, mock_agent, "agent_id",
                            mock_knowledge_graph, "Unsafe input"
                        )
...
class TestWebSocketEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_websocket_endpoint_normal_disconnect(
        self, mock_websocket, mock_knowledge_graph
    ):
        ...
        with patch('resync.api.chat._setup_websocket_session') as mock_setup:
            with patch('resync.api.chat._message_processing_loop') as mock_loop:
                ...
                await websocket_endpoint(
                    mock_websocket, "agent_id", mock_knowledge_graph
                )
    
    
    @pytest.mark.asyncio
    async def test_websocket_endpoint_llm_error(
        self, mock_websocket, mock_knowledge_graph
    ):
        """Teste erro de LLM"""
        with patch('resync.api.chat._setup_websocket_session') as mock_setup:
    ...
    @pytest.mark.asyncio
    async def test_websocket_endpoint_tool_error(
        self, mock_websocket, mock_knowledge_graph
    ):
        """Teste erro de tool"""
        with patch('resync.api.chat._setup_websocket_session') as mock_setup:
    ...
    @pytest.mark.asyncio
    async def test_websocket_endpoint_agent_error(
        self, mock_websocket, mock_knowledge_graph
    ):
        """Teste erro de agent"""
        with patch('resync.api.chat._setup_websocket_session') as mock_setup:
    ...
    @pytest.mark.asyncio
    async def test_websocket_endpoint_unexpected_error(
        self, mock_websocket, mock_knowledge_graph
    ):
    ...
    @pytest.mark.asyncio
    async def test_websocket_endpoint_setup_failure(
        self, mock_websocket, mock_knowledge_graph
    ):
    ...
    @pytest.mark.asyncio
    async def test_websocket_endpoint_complete_flow(
        self, mock_websocket, mock_knowledge_graph
    ):
