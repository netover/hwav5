...
class TestLightweightMetricsStore:
    ...
    async def test_record_with_tags(self, temp_db_path):
        ...
        await store.record(
            "tagged_metric", 1, MetricType.COUNTER, tags={"agent": "tws", "status": "success"}
        )

        value = store.get_counter("tagged_metric", tags={"agent": "tws", "status": "success"})
        assert value == 1

        # Different tags = different counter
        value_other = store.get_counter("tagged_metric", tags={"agent": "rag", "status": "success"})
    ...
    async def test_metric_point_tag_hash(self):
        ...
        point1 = MetricPoint(
            name="test", value=1, metric_type=MetricType.COUNTER, tags={"a": "1", "b": "2"}
        )
        point2 = MetricPoint(
            name="test",
            value=1,
        ...
        point3 = MetricPoint(name="test", value=1, metric_type=MetricType.COUNTER, tags={})
...
class TestContinualLearningMetrics:
    ...
    async def test_record_review_added(self, temp_db_path):
        ...
        added = store.get_counter(
            MetricNames.REVIEW_ADDED, tags={"primary_reason": "low_confidence"}
        )
        assert added == 1
    ...
    async def test_record_enrichment(self, temp_db_path):
        ...
        with_context = store.get_counter(
            MetricNames.ENRICHMENT_CONTEXT_ADDED, tags={"source": "learning_store"}
        )
    ...
    async def test_record_audit_processed(self, temp_db_path):
        ...
        processed = store.get_counter(
            MetricNames.AUDIT_PROCESSED, tags={"error_type": "incorrect_association"}
        )
        assert processed == 1
