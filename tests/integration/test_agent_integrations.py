...
def mock_tws_client():
    ...
    client.query_job_status = AsyncMock(return_value={
    ...
    client.query_workstation = AsyncMock(return_value={
        "workstation": "TWS_PROD_01",
        "status": "ACTIVE",
        "jobs_running": 5,
        "jobs_pending": 2
    ...
    client.analyze_logs = AsyncMock(return_value={
        ...
        with patch('resync.core.fastapi_di.get_service', return_value=mock_tws_error):
            with patch.object(manager, '_create_agent_instance') as mock_create:
                ...
                mock_agent.arun = AsyncMock(
                    return_value="Erro: Não foi possível conectar ao TWS"
                )
    ...
    async def test_agent_executes_multiple_tools(self, mock_tws_client, agent_config_task):
        ...
        with patch('resync.core.fastapi_di.get_service', return_value=mock_tws_client):
            with patch.object(manager, '_create_agent_instance') as mock_create:
                ...
                assert mock_tws_client.query_job_status.called or \
                       mock_tws_client.analyze_logs.called
...
class TestAgentMemoryIntegration:
    ...
    async def test_agent_stores_and_retrieves_context(self, mock_memory_service, agent_config_chat):
        ...
        with patch.object(manager, '_create_agent_instance') as mock_create:
            ...
            async def first_interaction(*args, **kwargs):
                await mock_memory_service.store_context("user_123", {
                    "query": args[0],
    ...
    async def test_agent_without_memory_doesnt_store_context(self, mock_memory_service, agent_config_task):
    ...
    async def test_agent_memory_persists_across_sessions(self, mock_memory_service, agent_config_chat):
        ...
        async def get_mock(user_id):
            return stored_data.get(user_id, {
                "previous_interactions": [],
                "user_preferences": {}
            })
        ...
        with patch.object(manager, '_create_agent_instance') as mock_create:
            ...
            async def arun_with_persist(*args, **kwargs):
                # Armazenar
                await mock_memory_service.store_context("user_456", {
                    "interactions": ["Query sobre job BACKUP_DAILY"]
                })
                # Recuperar
            ...
            # Act - Primeira sessão
            agent = await manager.get_or_create_agent(agent_config_chat.id)
            response1 = await agent.arun("Primeira query")
            
            # Act - Segunda sessão (mesmo agent, mesmo user)
            response2 = await agent.arun("Segunda query")
...
class TestAgentRAGIntegration:
    ...
    async def test_agent_combines_rag_with_llm(self, mock_rag_service, mock_litellm, agent_config_chat):
    ...
    async def test_agent_handles_rag_timeout(self, mock_rag_service, agent_config_chat):
        ...
        mock_rag_service.search = AsyncMock(
            side_effect=asyncio.TimeoutError("RAG timeout")
        )
        ...
        with patch('resync.core.fastapi_di.get_service', return_value=mock_tws_client):
            with patch.object(manager, '_create_agent_instance') as mock_create:
                ...
                async def complete_flow(*args, **kwargs):
                    ...
                    await mock_memory_service.store_context("user_789", {
                        "query": query,
                        "response": response
                    })
...
class TestAgentErrorHandling:
    ...
    async def test_agent_handles_all_services_down(self, agent_config_chat):
        ...
        mock_tws_error = AsyncMock()
        mock_tws_error.query_job_status = AsyncMock(
            side_effect=ConnectionError("TWS down")
        )
        
        mock_rag_error = AsyncMock()
        mock_rag_error.search = AsyncMock(
            side_effect=asyncio.TimeoutError("RAG timeout")
        )
