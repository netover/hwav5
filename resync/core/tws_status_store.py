...
class TWSStatusStore:
    ...
    async def update_job_status(self, job: JobStatus, snapshot_id: Optional[int] = None) -> TWSJobStatus:
    ...
    async def log_event(self, event_type: str, message: str, severity: str = "info",
                       job_name: Optional[str] = None, workstation: Optional[str] = None,
                       details: Optional[Dict[str, Any]] = None) -> TWSEvent:
        """Log an event."""
    ...
    async def get_events(self, limit: int = 100, severity: Optional[str] = None,
                        job_name: Optional[str] = None) -> List[TWSEvent]:
    ...
    async def acknowledge_event(self, event_id: int, acknowledged_by: str) -> Optional[TWSEvent]:
    ...
    async def get_patterns(self, job_name: Optional[str] = None, min_confidence: float = 0.5) -> List[TWSPattern]:
        """Get active patterns."""
    ...
    async def add_solution(self, problem_type: str, problem_description: str, solution: str,
                          job_name: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> TWSProblemSolution:
    ...
    async def find_solution(self, problem_type: str, job_name: Optional[str] = None) -> Optional[TWSProblemSolution]:
        """Find a solution for a problem."""
        return await self._store.find_solution(problem_type, job_name)
    
    async def record_solution_outcome(self, solution_id: int, success: bool) -> Optional[TWSProblemSolution]:
...
_instance: Optional[TWSStatusStore] = None
