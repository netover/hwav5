...
class GraphNode(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True, comment="Human-readable name"
    )

    # Properties as JSON
    properties_json: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Additional properties as JSON"
    )
...
class GraphEdge(Base):
    ...
    relation_type: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="Type of relationship"
    )

    # Edge weight (for algorithms like PageRank)
    weight: Mapped[float] = mapped_column(
        Float, default=1.0, comment="Edge weight for graph algorithms"
    )
    ...
    valid_from: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="When this relationship became valid"
    )
    valid_until: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="When this relationship expired (null = still valid)"
    )

    # Source tracking
    source: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="Where this edge came from"
    )

    # Confidence score (for LLM-extracted edges)
    confidence: Mapped[float] = mapped_column(
        Float, default=1.0, comment="Confidence score (1.0 = certain, 0.0 = uncertain)"
    )
