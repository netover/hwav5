...
class TWSRelation:
    ...
    def to_cypher(self) -> str:
        """Gera query Cypher para criar a relação."""
        props = ""
        if self.properties:
            prop_str = ", ".join(
        ...
        return f"""
        MATCH (a {{name: '{self.from_node}'}})
        MATCH (b {{name: '{self.to_node}'}})
        MERGE (a)-[r:{self.relation_type.value}{props}]->(b)
        SET r.weight = {self.weight}
        RETURN r
        """
...
class TWSRelationBuilder:
    ...
    def job_depends_on(
        ...
        self._relations.append(TWSRelation(
            from_node=job,
            to_node=depends_on,
...
def get_relation_types_info() -> Dict[str, Any]:
    ...
    return {
        ...
        "categories": {
            cat: [r.value for r in relations]
            for cat, relations in categories.items()
        },
