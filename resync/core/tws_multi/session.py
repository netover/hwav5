...
class TWSSession:
    ...
    selected_job_stream: Optional[str] = None
    ...
    def set_view(self, view: str, filters: Optional[Dict[str, Any]] = None):
...
class SessionManager:
    ...
    def create_session(
        ...
        logger.info(
            f"Session created: {session.id} for user {username} on {instance_name}"
        )
        
        return session
    
    def get_session(self, session_id: str) -> Optional[TWSSession]:
        """Get a session by ID."""
    ...
    def close_session(self, session_id: str):
        ...
        if session.instance_id in self._instance_sessions:
            self._instance_sessions[session.instance_id] = [
                sid for sid in self._instance_sessions[session.instance_id]
                if sid != session_id
            ]
