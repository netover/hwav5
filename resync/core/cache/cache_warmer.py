...
class CacheWarmer:
    ...
    async def warm_from_history(
        ...
        try:
            ...
            logger.info("Warm from history: feature pendente de implementação")
            return 0
                ...
                if self.cache and result:
                    ...
                    if hasattr(result, "documents"):
                        response_data["documents"] = [
                            d.dict() if hasattr(d, "dict") else d for d in result.documents
                        ]
                    ...
                    self._stats.queries_skipped += 1
                    logger.debug(f"Skipped (no cache available): {wq.query[:40]}...")
