...
class WarmingMetrics(BaseModel):
    ...
    last_warm: Optional[datetime] = Field(description="Último warming executado")
...
class MetricsStore:
    ...
    def uptime_hours(self) -> float:
        return (datetime.utcnow() - self._startup_time).total_seconds() / 3600
...
async def get_dashboard_metrics(
    ...
    try:
        ...
        warming_metrics = WarmingMetrics(
            last_warm=datetime.fromisoformat(stats["last_warm"]) if stats.get("last_warm") else None,
...
async def get_intent_distribution(
    ...
    distribution = {
        intent: count / total
        for intent, count in store._intent_counts.items()
    }
...
async def trigger_cache_warming(
    ...
    try:
        ...
        if warmer.is_warming:
            raise HTTPException(
                status_code=409,
                detail="Cache warming já está em progresso"
            )
        ...
        return {
            "status": "completed",
            "message": f"Cache warming executado com sucesso",
            "result": result,
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro no cache warming: {str(e)}"
        )
